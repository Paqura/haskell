...
{- HLINT ignore -}
bindBasicDomain domain = \getUrl pathname token -> getUrl domain pathname token

domainBuilder = bindBasicDomain "http://example.com"

getApiRequestBuilder geturl domainBuilder token = \pathname -> domainBuilder geturl pathname token
